        ...
        if (e.key === "w") {
            setInterval(() => {
                ...
                if (closestFoe != null && minCombinedDistance < 575) {
                    if (minCombinedDistance < 150) {
                        if (wrapPoint == null) {
                            wrapPoint = {
                                xx: unsafeWindow.snake.xx,
                                yy: unsafeWindow.snake.yy
                            }
                        }

                        console.log("WRAP -- " + minCombinedDistance);

                        unsafeWindow.xm = (wrapPoint.xx - Math.round(unsafeWindow.snake.xx));
                        unsafeWindow.ym = (wrapPoint.yy - Math.round(unsafeWindow.snake.yy));
                        return;
                    } else {
                        wrapPoint = null;
